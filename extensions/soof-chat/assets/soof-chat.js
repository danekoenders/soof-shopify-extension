class ChatBot extends HTMLElement {
    ...
    get styles() {
        const styleGuide = {
            primaryColor: this.serveData.chatbot?.primaryColor || '#083358',
            secondaryColor: this.serveData.chatbot?.secondaryColor || '#0D63A5',
            accentColor: '#FFD717',
            blackAccentColor: 'black',
            whiteAccentColor: 'white',
            dullWhiteAccentColor: '#f1f1f1',
            dullGrayAccentColor: '#d3d3d3',
            grayAccentColor: '#959595',
        ...
        return `
            ...
            input:disabled {
                background-color: ${styleGuide.dullGrayAccentColor};
                cursor: not-allowed;
            }
            ...
            .chat-header-buttons button {
                ...
                padding: 4px 10px;
                border-radius: 5px;
                border: 1px solid ${styleGuide.grayAccentColor};
                background: transparent;
                color: ${styleGuide.grayAccentColor};
                font-size: 0.8em;
                cursor: pointer;
            }
    ...
    get startChatMessage() {
        return `<p>Welkom bij ${this.serveData.shop?.customName || 'onze winkel'
            }! Vul hieronder je e-mailadres in om een chat te beginnen.</p>`;
    }

    get welcomeMessage() {
        return `Hey! 👋 Ik ben **${this.serveData.chatbot.customName || 'Soof'
            }**, de virtuele assistent🤖 van deze webwinkel. Ik ben in staat de meesten vragen voor je te beantwoorden. Stel gerust je eerste vraag of kies één van de **suggesties** hieronder!`;
    }
                    ...
                    options: [
                        {
                            ...
                            value: 'Ik wil graag de laatste bezorggegevens van mijn bestelling inzien.',
                        },
                ...
                ? `
                    <div class="chat-window">
                        <div class="chat-header">
                            <h4>${this.serveData.shop?.customName || 'Klantenservice'}</h4>
                            <span>Je chat met ${this.serveData.chatbot?.customName || 'Soof'}</span>
                        ...
                        <div class="chat-input">
                            <input id="chat-input-field" name="question" type="text" placeholder="Stel je vraag...">
                            <button id="send-button" ${this.sendDisabled ? 'disabled' : ''}>${this.sendButtonIcon
                }</button>
                        </div>
                    </div>
                `
        ...
        chatLog.innerHTML = this.messages
            .map((msg, msgIndex) => {
                    ...
                    <div class="message-wrapper ${msg.role}">
                        <div class="message">
                            <div class="email-input">
                                <input type="email" name="email" ${msg.email && `value="${msg.email}" `
                        }autocomplete="email" placeholder="E-mailadres">
                ...
                `;
                    ...
                    return `
                    <div class="message-wrapper ${msg.role}">
                        <div class="message"><p>Hier zijn de gegevens van je order:</p></div>
                    </div>
                    ...
                    <div class="message-wrapper ${msg.role}">
                        <div class="message">
                            ...
                            <a href="${msg.order.orderStatusUrl}" target="_blank">Details</a>
                        </div>
                        ${msg.options
                            ? `
                            <div class="options">
                                ...
                                (product) => `
                                        <li class="splide__slide">
                                            <div class="carousel-item">
                                                <div class="carousel-item-cover" style="background-image: url(${product.images[0]?.node.source || ''
                                    })"></div>
    ...
    async sendMessage({ message, functionObj }) {
        ...
        try {
            const response = await fetch(`${window.location.origin}/apps/${this.soofProxyURI}/assistant/chat`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            } else if (data.type === 'frontendFunction') {
                if (data.frontendFunction === "sendOrderTracking") {
                    this.messages.push({
                        role: 'assistant',
                        type: 'normal',
            ...
            } else if (data.type === 'orderTracking') {
                this.messages.push({
                    role: 'assistant',
                    type: data.type,
                    order: data.order,
